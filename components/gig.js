...
const HomeScreen = () => {
  ...
  useLayoutEffect(
    () =>
      onSnapshot(doc(db, "users_gig", user.uid), (snapshot) => {
        if (!snapshot.exists()) {
          navigation.navigate("gigmodal");
        }
      }),
    []
  );
  ...
  useEffect(() => {
    ...
    const fetchCards = async () => {
      const passes = await getDocs(
        collection(db, "users_gig", user.uid, "passes")
      ).then((snapshot) => snapshot.docs.map((doc) => doc.id));

      const swipes = await getDocs(
        collection(db, "users_gig", user.uid, "swipes")
      ).then((snapshot) => snapshot.docs.map((doc) => doc.id));
  ...
  const swipeLeft = (cardIndex) => {
    ...
    setDoc(doc(db, "users_gig", user.uid, "passes", userSwiped.id), userSwiped);
  };
  ...
  const swipeRight = async (cardIndex) => {
    ...
    const loggedInProfile = await (
      await getDoc(doc(db, "users_gig", user.uid))
    ).data();

    getDoc(doc(db, "users_gig", userSwiped.id, "swipes", user.uid)).then(
      (documentSnapshot) => {
        if (documentSnapshot.exists()) {
          console.log(`You've matched with ${userSwiped.displayName}`);

          setDoc(
            doc(db, "users_gig", user.uid, "swipes", userSwiped.id),
            userSwiped
          );
  ...
  return (
    <SafeAreaView style={{ flex: 1,backgroundColor: "white" }}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.iconContainer} onPress={() => navigation.navigate("gigmodal")}>
            ...
            card ? (
              ...
              <View style={styles.imageTextContainer}>
                <Text style={styles.imageTextName}>
                  {card.name_gig}, Age: {card.age_gig}Y
                </Text>
                <Text style={styles.imageTextJob}>
                  Profession: {card.profession}, Gig: {card.gig}
                </Text>
              </View>
            </View>
            ) : (
              ...
              >
                ...
                >
                  No GIG For You At the Moment
                </Text>
                <Image
                  style={{ height: 80, width: 80 }}
                  source={{uri : "https://firebasestorage.googleapis.com/v0/b/unify-v3-copy.appspot.com/o/tyrmwp0wndo-88%3A558?alt=media&token=fb305351-838d-43df-afaf-f95a73ac6eb1"}}
                />
              </View>
            )
          }
...
const styles = StyleSheet.create({
  ...
  iconContainer: {
    // Style your icon container as needed
  },
